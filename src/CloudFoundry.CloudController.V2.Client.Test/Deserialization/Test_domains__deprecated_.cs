//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

//
// This source code was auto-generated by cf-sdk-builder
//

using CloudFoundry.CloudController.V2.Client;
using CloudFoundry.CloudController.V2.Client.Data;
using Microsoft.CSharp;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using System.CodeDom.Compiler;

namespace CloudFoundry.CloudController.V2.Test.Deserialization
{
    [TestClass]
    [GeneratedCodeAttribute("cf-sdk-builder", "1.0.0.0")]
    public class DomainsDeprecatedTest
    {


        [TestMethod]
        public void TestCreateDomainOwnedByGivenOrganizationDeprecatedResponse()
        {
            string json = @"{
  ""metadata"": {
    ""guid"": ""0278169f-22bd-4765-9bf8-401d4965f035"",
    ""url"": ""/v2/domains/6bbff7a8-2e23-44a3-a7b7-93221880bd81"",
    ""created_at"": ""2016-04-11T13:33:17Z"",
    ""updated_at"": null
  },
  ""entity"": {
    ""name"": ""exmaple.com"",
    ""owning_organization_guid"": ""0278169f-22bd-4765-9bf8-401d4965f035"",
    ""shared_organizations"": [

    ],
    ""owning_organization_url"": ""/v2/organizations/07dd858a-8ebd-4d49-897c-ab2bf3491c5e"",
    ""spaces_url"": ""/v2/domains/6bbff7a8-2e23-44a3-a7b7-93221880bd81/spaces""
  }
}";

            CreateDomainOwnedByGivenOrganizationDeprecatedResponse obj = Utilities.DeserializeJson<CreateDomainOwnedByGivenOrganizationDeprecatedResponse>(json);

            Assert.AreEqual("0278169f-22bd-4765-9bf8-401d4965f035", TestUtil.ToTestableString(obj.EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/6bbff7a8-2e23-44a3-a7b7-93221880bd81", TestUtil.ToTestableString(obj.EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:33:17Z", TestUtil.ToTestableString(obj.EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("exmaple.com", TestUtil.ToTestableString(obj.Name), true);
            Assert.AreEqual("0278169f-22bd-4765-9bf8-401d4965f035", TestUtil.ToTestableString(obj.OwningOrganizationGuid), true);
            Assert.AreEqual("/v2/organizations/07dd858a-8ebd-4d49-897c-ab2bf3491c5e", TestUtil.ToTestableString(obj.OwningOrganizationUrl), true);
            Assert.AreEqual("/v2/domains/6bbff7a8-2e23-44a3-a7b7-93221880bd81/spaces", TestUtil.ToTestableString(obj.SpacesUrl), true);
        }

        [TestMethod]
        public void TestListAllSpacesForDomainDeprecatedResponse()
        {
            string json = @"{
  ""total_results"": 1,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""0978784a-30b4-4219-8853-e15094cd03d4"",
        ""url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d"",
        ""created_at"": ""2016-04-11T13:33:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""name"": ""name-2247"",
        ""organization_guid"": ""0978784a-30b4-4219-8853-e15094cd03d4"",
        ""space_quota_definition_guid"": null,
        ""allow_ssh"": true,
        ""organization_url"": ""/v2/organizations/54043f7c-2318-40e0-a690-b77e68da19d9"",
        ""developers_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/developers"",
        ""managers_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/managers"",
        ""auditors_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/auditors"",
        ""apps_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/apps"",
        ""routes_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/routes"",
        ""domains_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/domains"",
        ""service_instances_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/service_instances"",
        ""app_events_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/app_events"",
        ""events_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/events"",
        ""security_groups_url"": ""/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/security_groups""
      }
    }
  ]
}";

            PagedResponseCollection<ListAllSpacesForDomainDeprecatedResponse> page = Utilities.DeserializePage<ListAllSpacesForDomainDeprecatedResponse>(json, null);

            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("0978784a-30b4-4219-8853-e15094cd03d4", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:33:17Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("name-2247", TestUtil.ToTestableString(page[0].Name), true);
            Assert.AreEqual("0978784a-30b4-4219-8853-e15094cd03d4", TestUtil.ToTestableString(page[0].OrganizationGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].SpaceQuotaDefinitionGuid), true);
            Assert.AreEqual("true", TestUtil.ToTestableString(page[0].AllowSsh), true);
            Assert.AreEqual("/v2/organizations/54043f7c-2318-40e0-a690-b77e68da19d9", TestUtil.ToTestableString(page[0].OrganizationUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/developers", TestUtil.ToTestableString(page[0].DevelopersUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/managers", TestUtil.ToTestableString(page[0].ManagersUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/auditors", TestUtil.ToTestableString(page[0].AuditorsUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/apps", TestUtil.ToTestableString(page[0].AppsUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/routes", TestUtil.ToTestableString(page[0].RoutesUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/domains", TestUtil.ToTestableString(page[0].DomainsUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/service_instances", TestUtil.ToTestableString(page[0].ServiceInstancesUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/app_events", TestUtil.ToTestableString(page[0].AppEventsUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/events", TestUtil.ToTestableString(page[0].EventsUrl), true);
            Assert.AreEqual("/v2/spaces/98a3b978-4065-46c6-a38d-8090754b6c4d/security_groups", TestUtil.ToTestableString(page[0].SecurityGroupsUrl), true);
        }

        [TestMethod]
        public void TestRetrieveDomainDeprecatedResponse()
        {
            string json = @"{
  ""metadata"": {
    ""guid"": ""f6a2fff1-3d22-4853-b27b-079e343b1e04"",
    ""url"": ""/v2/domains/5333bcad-8bd7-4d9d-9358-4624a150f952"",
    ""created_at"": ""2016-04-11T13:33:17Z"",
    ""updated_at"": null
  },
  ""entity"": {
    ""name"": ""domain-80.example.com"",
    ""router_group_guid"": null,
    ""router_group_type"": null
  }
}";

            RetrieveDomainDeprecatedResponse obj = Utilities.DeserializeJson<RetrieveDomainDeprecatedResponse>(json);

            Assert.AreEqual("f6a2fff1-3d22-4853-b27b-079e343b1e04", TestUtil.ToTestableString(obj.EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/5333bcad-8bd7-4d9d-9358-4624a150f952", TestUtil.ToTestableString(obj.EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:33:17Z", TestUtil.ToTestableString(obj.EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("domain-80.example.com", TestUtil.ToTestableString(obj.Name), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.RouterGroupGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.RouterGroupType), true);
        }

        [TestMethod]
        public void TestCreateSharedDomainDeprecatedResponse()
        {
            string json = @"{
  ""metadata"": {
    ""guid"": ""ab5db1b7-961c-41c8-a1c8-0230932f2551"",
    ""url"": ""/v2/domains/66b4d917-fe02-425c-ae6f-7013e013a279"",
    ""created_at"": ""2016-04-11T13:33:17Z"",
    ""updated_at"": null
  },
  ""entity"": {
    ""name"": ""example.com"",
    ""owning_organization_guid"": null,
    ""shared_organizations"": [

    ]
  }
}";

            CreateSharedDomainDeprecatedResponse obj = Utilities.DeserializeJson<CreateSharedDomainDeprecatedResponse>(json);

            Assert.AreEqual("ab5db1b7-961c-41c8-a1c8-0230932f2551", TestUtil.ToTestableString(obj.EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/66b4d917-fe02-425c-ae6f-7013e013a279", TestUtil.ToTestableString(obj.EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:33:17Z", TestUtil.ToTestableString(obj.EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("example.com", TestUtil.ToTestableString(obj.Name), true);
            Assert.AreEqual("", TestUtil.ToTestableString(obj.OwningOrganizationGuid), true);
        }

        [TestMethod]
        public void TestListAllDomainsDeprecatedResponse()
        {
            string json = @"{
  ""total_results"": 4,
  ""total_pages"": 1,
  ""prev_url"": null,
  ""next_url"": null,
  ""resources"": [
    {
      ""metadata"": {
        ""guid"": ""5c194924-434a-4203-a21c-792d80d5a6b0"",
        ""url"": ""/v2/domains/d41c9588-c845-4b1f-9482-53975b920144"",
        ""created_at"": ""2016-04-11T13:32:56Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""name"": ""customer-app-domain1.com"",
        ""router_group_guid"": null,
        ""router_group_type"": null
      }
    },
    {
      ""metadata"": {
        ""guid"": ""5c194924-434a-4203-a21c-792d80d5a6b0"",
        ""url"": ""/v2/domains/f1aeadcc-9b1f-4216-8469-a170c94544d5"",
        ""created_at"": ""2016-04-11T13:32:56Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""name"": ""customer-app-domain2.com"",
        ""router_group_guid"": null,
        ""router_group_type"": null
      }
    },
    {
      ""metadata"": {
        ""guid"": ""5c194924-434a-4203-a21c-792d80d5a6b0"",
        ""url"": ""/v2/domains/d2a4ac05-b3e3-4753-9300-8469ded744d3"",
        ""created_at"": ""2016-04-11T13:32:56Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""name"": ""vcap.me"",
        ""owning_organization_guid"": ""5c194924-434a-4203-a21c-792d80d5a6b0"",
        ""owning_organization_url"": ""/v2/organizations/a9794d99-0f12-4922-9e83-c1062256f034"",
        ""spaces_url"": ""/v2/domains/d2a4ac05-b3e3-4753-9300-8469ded744d3/spaces""
      }
    },
    {
      ""metadata"": {
        ""guid"": ""5c194924-434a-4203-a21c-792d80d5a6b0"",
        ""url"": ""/v2/domains/567db471-9ead-4419-9445-cd2823b97fce"",
        ""created_at"": ""2016-04-11T13:33:17Z"",
        ""updated_at"": null
      },
      ""entity"": {
        ""name"": ""domain-76.example.com"",
        ""router_group_guid"": null,
        ""router_group_type"": null
      }
    }
  ]
}";

            PagedResponseCollection<ListAllDomainsDeprecatedResponse> page = Utilities.DeserializePage<ListAllDomainsDeprecatedResponse>(json, null);

            Assert.AreEqual("4", TestUtil.ToTestableString(page.Properties.TotalResults), true);
            Assert.AreEqual("1", TestUtil.ToTestableString(page.Properties.TotalPages), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.PreviousUrl), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page.Properties.NextUrl), true);
            Assert.AreEqual("5c194924-434a-4203-a21c-792d80d5a6b0", TestUtil.ToTestableString(page[0].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/d41c9588-c845-4b1f-9482-53975b920144", TestUtil.ToTestableString(page[0].EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:32:56Z", TestUtil.ToTestableString(page[0].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("customer-app-domain1.com", TestUtil.ToTestableString(page[0].Name), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].RouterGroupGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[0].RouterGroupType), true);
            Assert.AreEqual("5c194924-434a-4203-a21c-792d80d5a6b0", TestUtil.ToTestableString(page[1].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/f1aeadcc-9b1f-4216-8469-a170c94544d5", TestUtil.ToTestableString(page[1].EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:32:56Z", TestUtil.ToTestableString(page[1].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[1].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("customer-app-domain2.com", TestUtil.ToTestableString(page[1].Name), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[1].RouterGroupGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[1].RouterGroupType), true);
            Assert.AreEqual("5c194924-434a-4203-a21c-792d80d5a6b0", TestUtil.ToTestableString(page[2].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/d2a4ac05-b3e3-4753-9300-8469ded744d3", TestUtil.ToTestableString(page[2].EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:32:56Z", TestUtil.ToTestableString(page[2].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[2].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("vcap.me", TestUtil.ToTestableString(page[2].Name), true);
            Assert.AreEqual("5c194924-434a-4203-a21c-792d80d5a6b0", TestUtil.ToTestableString(page[2].OwningOrganizationGuid), true);
            Assert.AreEqual("/v2/organizations/a9794d99-0f12-4922-9e83-c1062256f034", TestUtil.ToTestableString(page[2].OwningOrganizationUrl), true);
            Assert.AreEqual("/v2/domains/d2a4ac05-b3e3-4753-9300-8469ded744d3/spaces", TestUtil.ToTestableString(page[2].SpacesUrl), true);
            Assert.AreEqual("5c194924-434a-4203-a21c-792d80d5a6b0", TestUtil.ToTestableString(page[3].EntityMetadata.Guid), true);
            Assert.AreEqual("/v2/domains/567db471-9ead-4419-9445-cd2823b97fce", TestUtil.ToTestableString(page[3].EntityMetadata.Url), true);
            Assert.AreEqual("2016-04-11T13:33:17Z", TestUtil.ToTestableString(page[3].EntityMetadata.CreatedAt), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[3].EntityMetadata.UpdatedAt), true);
            Assert.AreEqual("domain-76.example.com", TestUtil.ToTestableString(page[3].Name), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[3].RouterGroupGuid), true);
            Assert.AreEqual("", TestUtil.ToTestableString(page[3].RouterGroupType), true);
        }
    }
}
